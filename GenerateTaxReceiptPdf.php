...
<p>Thank you for your support of W. H. Taylorӳ PTA. Because of your generous donations, our PTA was able to help fund many important services for our school as well as Taylor Families.  </p>
<ul>
    ...
    <li>Class fieldtrips are open to all students, whether or not their family has the money to pay the fieldtrip fees. When a childӳ family is unable to pay, W. H. Taylor PTA fills in the gap so every child can participate.</li>
...
$mpdf->WriteHTML('<p>Donor: '.$BusinessName.'</p>');
$mpdf->WriteHTML('<p>Donations</p>');
$mpdf->WriteHTML('<ul>');
						if ($result->num_rows > 0)
						{
							...
							$TotalValue += $row["RetailValue"];
								$mpdf->WriteHTML('<li>'.$row["Description"].', $'.$row["RetailValue"].'</li>');
							}
						}
						else{
							$mpdf->WriteHTML('No Donations Found.');
						}
$mpdf->WriteHTML('</ul>');
